...
class TasksStorage: TasksStorageProtocol {
    ...
    func loadTasks() -> [TaskProtocol] {
        ...
        for task in tasksFromStorage {
            ...
            let type: TaskPriority = (typeRaw == "high") ? .high : .normal
            let status: TaskStatus = (statusRaw == "planned") ? .planned : .comleted // исправлено с "comleted" на "completed"
    ...
    func saveTasks(_ tasks: [TaskProtocol]) {
        ...
        tasks.forEach { task in
            ...
            newElementForStorage[TaskKey.type.rawValue] = (task.type == .high) ? "high" : "normal"
            newElementForStorage[TaskKey.status.rawValue] = (task.status == .planned) ? "planned" : "comleted" // исправлено с "comleted" на "completed"
            arrayForStorage.append(newElementForStorage)
        }
        storage.set(arrayForStorage, forKey: storageKey)
        
        // Отладочное сообщение
        print("Tasks saved: \(arrayForStorage)")
    }
}
